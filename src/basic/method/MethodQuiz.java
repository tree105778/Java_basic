...
//        if (indexOf(searchTarget) == -1) return false;
//        else return true;
...
//        if (targetIndex >= 0 && targetIndex < foods.length) return false;
//        else return true;
    ...
    static void insert(int targetIndex, String newFoodName) {
        ...
        String[] temp = makeNewArray(1);
        for (int i = 0; i < foods.length; i++) {
            if (i < targetIndex) temp[i] = foods[i];
            else temp[i + 1] = foods[i];
        }
    ...
    static String[] copy(int size) {
        ...
        // 반복문의 횟수를 size로 판단해서 다르게 상황에 따라 다르게 지정.
        // size가 0보다 작다? -> temp가 foods보다 더 작다 -> 끝 범위는 temp.length
        // size가 0보다 크다? -> temp가 foods보다 더 크다 -> 끝 범위는 foods.length
